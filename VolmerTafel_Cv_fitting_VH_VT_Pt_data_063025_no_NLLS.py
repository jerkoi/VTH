...
# Create interpolation function for the model current
model_interp = interp1d(V, curr1, kind='linear', fill_value='extrapolate')

# Dictionary to store the interpolated values
I_model_interp_dict = {}
V_model_interp_dict = {}
...
for i in range(2, 9):
    ...
    I_exp = globals()[f"I_0{i}"]

    # Interpolate model current to experimental voltages
    I_model_interp = model_interp(V_exp)

    # Store interpolated values in new variables
    globals()[f"V_model_0{i}"] = V_exp
    globals()[f"I_model_0{i}"] = I_model_interp

    # Optional: also store in dict if you want to keep things clean
    V_model_interp_dict[f"V_model_0{i}"] = V_exp
    I_model_interp_dict[f"I_model_0{i}"] = I_model_interp

    # You can also calculate std deviation here
    std = np.std(I_model_interp - I_exp)
    print(f"Standard deviation for set 0{i}: {std:.3e}")

    # # Plot
    # plt.figure(figsize=(7, 5))
    # plt.plot(V_exp, I_exp, 'o', label='Experimental', markersize=5)
    # plt.plot(V_exp, I_model_interp, '-', label='Model (Interpolated)', linewidth=2)
    # plt.title(f"Set 0{i} Comparison\nStandard Deviation = {std:.3e}")
