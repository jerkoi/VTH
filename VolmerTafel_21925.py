...
def rates(t, theta):
    ...
    ##Volmer
    r_V = k1 * (thetaA_star ** (1 - beta)) * (thetaA_H ** beta) * np.exp(beta * GHad / RT) * (np.exp(-(beta) * F * (V - U_V) / RT) - np.exp((1 - beta) * F * (V - U_V) / RT))

    ##Tafel
    i0 = (k1 * F * theta_max * (sqrtP**beta) * (thetaA_H**(1 - beta)))/(1 + sqrtP)
