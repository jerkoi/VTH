...
from VolmerTafel_Cv_fitting_VH_VT_finished import V_model
...
def model(t, theta0):
    ...
    I_model = np.empty(len(t), dtype=object)
    ...
    I_model = volmer_rate * -F * 1000 #finds max current density
    return V_model, I_model
...
###########################################################################################################################
############################################################################################################################
def run_model(GHad_value):
    global GHad
    GHad = GHad_value
    theta0 = [0.01, 0.99]
    V_model, curr = model(t, theta0)
    return V_model, curr

def objective(GHad_try):
    #make "pristine" not hard coded
    V_model, I_model = run_model(GHad_try[0]) #update I_model name
    mask_model = (V_model >= min(Pristine_experi_V)) & (V_model <= max(Pristine_experi_V))
    mask_experi = (Pristine_experi_V >= min(V_model)) & (Pristine_experi_V <= max(V_model))
    V_common = V_model[mask_model]
    I_model = I_model[mask_model]
    interp_experi = interp1d(Pristine_experi_V[mask_experi], Pristine_experi_I[mask_experi], fill_value='extrapolate')
    #plot V comm and V_model, I_model to ensure they are same values
    I_experi_interp = interp_experi(V_common)
    return np.sum((I_model - I_experi_interp) ** 2)

plt.figure(figsize=(8, 6))
plt.plot(Pristine_experi_V, V_model)
plt.show()

# Minimize
initial_guess = [-0.3]
result = minimize(objective, x0=initial_guess, method='Nelder-Mead')
optimal_GHad = result.x[0]
print(f"Optimal GHad (J/mol): {optimal_GHad:.4e}")

# Final model
V_fit, I_fit = run_model(optimal_GHad)
...
# # Plot
# plt.figure(figsize=(8, 6))
# plt.plot(Pristine_experi_V, Pristine_experi_I, label="Experimental", color="black")
# plt.plot(V_fit[10:20000], I_fit[10:20000], label="Model Fit", color="blue")
# plt.xlabel("Voltage vs. RHE (V)")
# plt.ylabel("Current Density (mA/cm²)")
# plt.title("Nonlinear Least Squares Fit of GHad")
# plt.legend()
# plt.grid()
# plt.tight_layout()
# plt.show()
...
#
# fig, axs = plt.subplots(figsize=(8, 10))
# axs.plot(V_fit[10:20000], I_fit[10:20000], 'b', label='Model Data')
...
# ax.plot(np.abs(I_fit[10:20000]), V_fit[10:20000], 'b', label='Model Data')
...
common_Vmin = max(min(V_fit[10:20000]), min(Pristine_experi_V))
common_Vmax = min(max(V_fit[10:20000]), max(Pristine_experi_V))

# Restrict data to overlapping region
mask_model = (V_fit[10:20000] >= common_Vmin) & (V_fit[10:20000] <= common_Vmax)
mask_experi = (Pristine_experi_V >= common_Vmin) & (Pristine_experi_V <= common_Vmax)

V_model = V_fit[10:20000][mask_model]
I_model = I_fit[10:20000][mask_model]
