...
I_02 = df_02.iloc[311:746, 9] / 0.0929  + 0.102 #converting to mA/cm^2
...
model_mask = V_model <= 0
V_model_masked = V_model[model_mask]
curr_model_masked = curr_model[model_mask]

#make the mask here
scan_mask = V_02 <= 0 #insert voltage of whichever scan you want
...
I_model_interp_pristine = interp_func(V_02_masked) #in the interp_func it should be the voltage mask of whatever scan you want
print("Min and Max, I_02 Mask", np.min(I_02_masked), np.max(I_02_masked))
print("Min and Max, I_model_interp_pristine", np.min(I_model_interp_pristine), np.max(I_model_interp_pristine))

I_model_interp_masked = I_model_interp_pristine <= 0
I_model_interp_pristine_masked = I_model_interp_pristine[I_model_interp_masked]
print("Min and Max, I_model_interp_pristine_masked", np.min(I_model_interp_pristine_masked), np.max(I_model_interp_pristine_masked))
r2_scan = r_squared(I_02_masked, I_model_interp_pristine) #in the r^2 function it should be the current masked
print(f"R² for Pristine vs Model: {r2_scan:.4f}")
...
axs.plot(V_model[10:], curr_model[10:], 'b', label='Model Data')
...
ax.plot(np.abs(curr_model[10:]), V_model[10:], 'b', label='Model Data')
