...
def Grxn(theta_temp):
    ...
    return(dGk_list)
'''
Needs to call Gsites each time because the G of each site type is affected by surround occupancies
'''
...
def site_balance(theta_temp, t, oprams):
    ...
    dtheta_dt = [-r[0]/cmax, r[0]/cmax] # [forward Volmer, reverse Volmer]
    return (dtheta_dt)
...
for i in range(ts):
    theta_temp = theta_complete_list[i,:] #load solution as function of 't'
    Vapp = potential(t[i])   #calculate pontential 't'
    r = rates(theta_temp, Vapp)
    curr_kin[i] = 1000*F*(r[0])  #kinetic current is the summation of rates of all electrochemical steps
...
plt.plot(V[0:ts], curr_tot[0:ts], "g")
