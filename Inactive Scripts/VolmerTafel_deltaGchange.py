...
def G_func(t):
    GHad = F * np.where((t // 10) % 2 == 0, -0.15, -0.20)
    return GHad
...
def eqpot(theta):
    ...
    GHad = G_func(t) #call function to find GHad for given t
...
def sitebal_r0(t, theta):
    ...
    r_V, r_T = rates_r0(t, theta)  # r_V and r_T are scalars
    thetaStar_rate = (-2 * r_V + r_T) / cmax
    thetaH_rate = (2 * r_V - r_T) / cmax
    return np.array([thetaStar_rate, thetaH_rate])  # Return as a 1D array with length 2
