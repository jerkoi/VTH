...
r2_pristine = r_squared(Pristine_experi_I_masked[2:], I_model_interp_pristine[2:])
...
axs.plot(Pristine_experi_V_masked[cut:], I_model_interp_pristine[cut:], 'b', label='Model Data')
axs.plot(Pristine_experi_V_masked[cut:], Pristine_experi_I_masked[cut:], 'r', label='Pristine Experimental Data')
axs.set_xlim(None, 0.0)
axs.set_xlabel('Voltage vs. RHE (V)')
axs.set_ylim(None, 0.0)
axs.set_xlim(None, 0.0)
axs.set_ylabel('Kinetic current (mA/cm2)')
axs.set_title(fr'Kinetic Current vs Voltage, Good Sample vs Model, $R²$ = {r2_pristine:.4f}')
axs.grid()
axs.legend()
plt.show()
#
...
ax.plot(np.abs(I_model_interp_pristine[cut:]), Pristine_experi_V_masked[cut:], 'b', label='Model Data')
ax.plot(np.abs(Pristine_experi_I_masked[cut:]), Pristine_experi_V_masked[cut:], 'r', label='Pristine Experimental Data')
ax.set_xlabel('Log Kinetic current (mA/cm2)')
ax.set_ylabel('Voltage vs. RHE (V)')
ax.set_title(fr'Log Kinetic Current vs Voltage, Good Sample vs Model, $R²$ = {r2_pristine:.4f}')
ax.semilogx()
