...
if do_dynamic_ghad:
    ...
    def equil_theta(GHad, t):
        V = potential(0)
        thetaAH_equil = 1 / (1 + np.exp((F * V + GHad) / RT))
        thetaA_star_equil = 1 - thetaAH_equil

        return thetaAH_equil, thetaA_star_equil

    #equil potential
    def eqpot(theta, GHad):
        thetaA_star_equil, thetaAH_equil = equil_theta(GHad, t)      #new equilibrium surface coverage
        ##Volmer
        U_V = (-GHad / F) + (RT * np.log(thetaA_star_equil  / thetaAH_equil)) / F
        ...
        if mechanism_choice == 0:
            U_11 = GHad / F
            U_12 = (RT / F) * np.log(thetaAH_equil / thetaA_star_equil)
            U_H = U_11 + U_12
    ...
    def rates_r0(t, theta):
        ...
        r_V = k_V * (thetaA_star_equil ** (1 - beta[0])) * (thetaAH_equil ** beta[0]) * np.exp(beta[0] * GHad / RT) * (
                    np.exp(-(beta[0]) * F * (V - U_V) / RT) - np.exp((1 - beta[0]) * F * (V - U_V) / RT))

        r_T = 0
        if mechanism_choice == 0:
            T_1 = (thetaAH_equil ** 2)
            T_2 = (partialPH2 * (thetaA_star_equil ** 2) * np.exp((-2 * GHad) / RT))
            r_T = k_T * (T_1 - T_2)
        ...
        if mechanism_choice == 1:
            j1 = k_H * np.exp(-beta[1] * GHad / RT) * thetaA_star_equil ** beta[1] * thetaAH_equil ** (1 - beta[1])
    ...
    #plt.plot(t, theta_at_t, label="ODE theta_H")
    plt.plot(t, thetaH_equil_array, '--', label="Equilibrium theta_H")
    plt.legend()
